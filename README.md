...
- `/api/generate` OpenAI-backed conversation endpoint

## Local setup

1. Use Node 20 or newer.
2. Run `npm install`.
3. Copy `.env.example` to `.env.local` and fill in `OPENAI_API_KEY`.
...
| --- | --- | --- |
| `OPENAI_API_KEY` | Yes | Server-only key for the Responses API. |
...
2. Add every value in `.env.example` to the appropriate Vercel environment. Keep `OPENAI_API_KEY` server-only.
...
Run `npm test` for payload validation, renderer-safe response mapping, rate limiting, OpenAI failure handling, and HTTP error states. Run `npm run build` with non-production test variables to validate the Vercel output.
