...
export const SCENES = ["daily", "group", "work", "support"];
export const TONES = ["natural", "casual", "professional", "dramatic"];
...
export function validateGeneratePayload(input) {
  ...
  if (!hasOnlyKeys(input, ["topic", "scene", "tone", "language", "rounds"])) {
  ...
  if (!TONES.includes(input.tone)) return { ok: false, error: "Tone is invalid." };
  ...
  return { ok: true, value: { topic, scene: input.scene, tone: input.tone, language: input.language, rounds: input.rounds } };
}
...
export function normalizeConversation(value, request) {
  ...
  if (!Array.isArray(participants) || participants.length < 2 || participants.length > 4) throw new Error("Model response has invalid participants.");
...
export function buildConversationInstructions(request) {
  return [
    "You write short, natural chat conversations for a visual conversation generator.",
    `Write every message in ${LANGUAGE_NAMES[request.language]}.`,
    `The scene is ${request.scene}; the tone is ${request.tone}.`,
    `Create exactly ${request.rounds} messages. Keep each message under 180 characters.`,
    "Use 2 to 4 participants. Include exactly one participant with side 'me'.",
    ...
    "Return only the schema-compliant object."
  ].join(" ");
}
